...
TEST(SchedulerTest, Multiple) {
    {
        ...
        combo2::node device{network, common::make_tagged_tuple<uid,devtag>(7,times_t{0.0})};
        double d;
        d = device.next();
        device.update();
    ...
    }
    {
        ...
        combo4::node device{network, common::make_tagged_tuple<uid,devtag>(7,times_t{0.0})};
        double d;
        d = device.next();
        device.update();
        EXPECT_NEAR(1.5, d, 1e-6);
