...
TEST(DistributionTest, Constant) {
    ...
    d = distr(rnd);
    EXPECT_DOUBLE_EQ(5.0, d);
    fcpp::constant_distribution<int, 4> dint;
    int i;
    i = dint(rnd);
    EXPECT_EQ(4, i);
    i = dint(rnd);
    EXPECT_EQ(4, i);
    fcpp::constant_distribution<double, 52, 10> ddouble;
    d = ddouble(rnd);
...
TEST(DistributionTest, Uniform) {
    std::mt19937 rnd(42);
    fcpp::uniform_distribution<d5, d1> distr;
    double d;
    d = distr(rnd);
    ...
    fcpp::uniform_d<double, 5, 1> dratio;
    d = dratio(rnd);
    EXPECT_NEAR(5.0, d, 1.74);
    d = dratio(rnd);
    EXPECT_NEAR(5.0, d, 1.74);
...
TEST(DistributionTest, Normal) {
    std::mt19937 rnd(42);
    fcpp::normal_distribution<d5, d1> distr;
    double d;
    d = distr(rnd);
    ...
    for (int i=0; i<10000; ++i)
        d += distr(rnd);
    EXPECT_NEAR(50000.0, d, 300.0);
    fcpp::normal_d<double, 5, 1> dratio;
    d = dratio(rnd);
    EXPECT_NEAR(5.0, d, 3.0);
    d = dratio(rnd);
    EXPECT_NEAR(5.0, d, 3.0);
...
TEST(DistributionTest, Exponential) {
    std::mt19937 rnd(42);
    fcpp::exponential_distribution<d5, d5> distr;
    double d;
    d = distr(rnd);
    ...
    fcpp::exponential_d<double, 5, 5> dratio;
    d = dratio(rnd);
    EXPECT_NEAR(10.0, d, 10.0);
    d = dratio(rnd);
    EXPECT_NEAR(10.0, d, 10.0);
...
TEST(DistributionTest, Weibull) {
    std::mt19937 rnd(42);
    fcpp::weibull_distribution<d5, d1> distr;
    double d;
    d = distr(rnd);
    ...
    for (int i=0; i<10000; ++i)
        d += distr(rnd);
    EXPECT_NEAR(50000.0, d, 300.0);
    fcpp::weibull_d<double, 5, 1> dratio;
    d = dratio(rnd);
    EXPECT_NEAR(5.0, d, 5.0);
    d = dratio(rnd);
    EXPECT_NEAR(5.0, d, 5.0);
...
TEST(DistributionTest, Positive) {
    std::mt19937 rnd(42);
    fcpp::make_positive<fcpp::uniform_distribution<d1, d5>> distr;
    double d;
    d = distr(rnd);
    EXPECT_NEAR(5.0, d, 5.0);
    d = distr(rnd);
